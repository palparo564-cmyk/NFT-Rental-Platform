...
export const CONTRACT_ADDRESS =
  "CBUKIZIJA36UUIDSSQO7SIYWRLXRDNHUDAQ4UBMRTMYBE7IIAUTIMZVZ";
...
// ============================================================
// NFT Rental Platform — Contract Methods
// ============================================================

/**
 * List an NFT for rent.
 * Calls: list_nft(owner: Address, nft_id: Symbol, price_per_day: i128)
 */
export async function listNFT(
  ...
  return callContract(
    "list_nft",
    [toScValAddress(caller), toScValSymbol(nftId), toScValI128(pricePerDay)],
    caller,
    true
  );
}
...
/**
 * Rent an NFT.
 * Calls: rent_nft(renter: Address, nft_id: Symbol, days: i128)
 */
export async function rentNFT(
  ...
  return callContract(
    "rent_nft",
    [toScValAddress(caller), toScValSymbol(nftId), toScValI128(days)],
    caller,
    true
  );
}
...
/**
 * Return a rented NFT.
 * Calls: return_nft(nft_id: Symbol)
 */
...
/**
 * Get NFT rental status (read-only).
 * Calls: get_status(nft_id: Symbol) -> (Address, i128, bool)
 * Returns: { 0: owner address, 1: price per day, 2: is rented }
 */
export async function getNFTStatus(
  nftId: string,
  caller?: string
) {
  return readContract(
